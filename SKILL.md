---
name: skill-radoute
description: 元技能路由器。在一次会话内发现、选择、调用并切换其它 skill，维护跨 skill 的共享上下文与可回溯调用链。当任务需要多个技能协作、不确定该用哪个技能、当前技能失败需要换一个、要求记录/审计技能调用过程、或本地没有合适技能需要从网络检索安装时使用。触发词：技能路由、skill router、该用哪个技能、该用什么技能、用哪个工具好、帮我选个技能、换个技能试试、切换技能、切换工具、编排多个 skill、调用链、找不到合适的技能、技能注册表、上下文传递给下一个技能。
---
...
路由可以「越用越准」：当某个任务被路由到不合适的技能、或模型否决了 top1 之后，把这次修正记下来，下次相似任务打分时会自动加权——被选中的技能加分、被否决的技能减分，让路由结果贴合你的使用习惯。

```bash
"$PY" "$S/router.py" feedback list     # 列出所有反馈（初始为空）
"$PY" "$S/router.py" feedback stats    # 统计：总记录数 + 覆盖任务数
"$PY" "$S/router.py" feedback clear    # 清空全部反馈
```

**数据落盘与加权规则**：
- 反馈只存本机 `~/.workbuddy/feedback.json`（可用环境变量 `SKILL_RADOUTE_FEEDBACK` 改路径），**绝不上传、不外发**，仅用于本地路由打分优化。
- 打分时若当前任务与某条反馈的 `task` 相似度 > 0.8：该反馈的 `chosen` 技能 **+1.5**，`excluded` 技能 **−2.0**；系数受 `weight` 影响（1.0 全量、0.5 减半）。
- 记录/清空反馈会使路由决策缓存自动失效，旧权重不会残留。
- 隐私承诺：数据完全由你掌控，可随时用 `router.py feedback clear` 一键清空，删除后不再影响任何路由结果。
